			...
			public boolean onCommand(CommandSender s, Command c, String label, String[] args) {
				if (args.length < 1) {
					...
					try {
						if(rs2.next()) {
							if (ActualPrice == 0) {
								...
								s.sendMessage(Prefix + "Â§cPackage names are case sensitive.");
								return true;
							}
					...
					if (!DonationPoints.permission.has(s,  "donationpoints.purge")) {
						...
						s.sendMessage(Prefix + " Â§cAll Empty Accounts Purged.");
					}
					...
					if (args[1].equalsIgnoreCase("info")) {
						...
						s.sendMessage("-----Â§e" + packName + " InfoÂ§f-----");
						s.sendMessage("Â§aPackage Name:Â§3 " + packName);
						s.sendMessage("Â§aPrice:Â§3 " + price + "0");
						s.sendMessage("Â§aDescription:Â§3 " + description);
						if (plugin.getConfig().getBoolean("packages." + packName + ".expires") == false) {
							s.sendMessage("Â§aExpires: Â§3Never");
							...
							if (expiretime == 1) {
								s.sendMessage("Â§aExpires After: Â§3" + expiretime + " Day");
							} else if (expiretime != 1) {
								s.sendMessage("Â§aExpires After: Â§3" + expiretime + " Days");
							}
						}
						if (plugin.getConfig().getBoolean("packages." + packName + ".requireprerequisite") == false) {
							s.sendMessage("Â§aPrerequisite: Â§3None");
						} else if (plugin.getConfig().getBoolean("packages." + packName + ".requireprerequisite") == true) {
							s.sendMessage("Â§aPrerequisite: Â§3" + plugin.getConfig().getString("packages." + packName + ".prerequisite"));
						}
					}
					if (args[1].equalsIgnoreCase("list")) {
						...
						s.sendMessage(Prefix + "Â§3Available Packages: Â§a" + packages.toString());
						return true;
					...
					s.sendMessage(Prefix + "Â§cDeleted Â§3" + accountName + "'s Â§caccount.");
					...
					s.sendMessage(Prefix + "Â§cClick the Item Frame you would like to link Â§3" + packName + " Â§cto.");
